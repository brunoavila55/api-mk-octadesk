...
| POST | `/v1/llm-classifica-mensagem` | corpo `{"mensagem": "..."}` | Classifica a mensagem do cliente em um setor (`vendas`, `renovacao`, `ampliacao`, `endereco`, `titular`, `cancelamento`, `financeiro`, `suporte` ou `atendimento`) via LLM |
...
# Classificação LLM — espera 200 com dados.destino em {vendas,renovacao,ampliacao,endereco,titular,cancelamento,financeiro,suporte,atendimento}
